...
require("dotenv").config();
  ...
  // DB + other models
  require("./db/conn");
  // require("./Model/order.traking"); // already required above via require var
  ...
  // Static folders (adjust path as required)
  app.use(express.static("public"));
  app.use("/images", express.static("D:\\Job\\Telve\\tolvv\\public\\images"));

  // Temporary in-memory OTP store (small-scale use only)
  const tempOtp = {};
  const OTP_EXPIRY_TIME = 5 * 60 * 1000; // 5 minutes

  // Cleanup expired OTPs periodically to avoid memory leak
  setInterval(() => {
    const now = Date.now();
    for (const mobile in tempOtp) {
      if (tempOtp[mobile].expiresAt < now) delete tempOtp[mobile];
    }
  ...
  const authenticate = (req, res, next) => {
    ...
    const token = header?.replace?.("Bearer ", "") || header;
    if (!token) return res.status(401).json({ message: "No token provided" });
  ...
  app.post("/send-otp", async (req, res) => {
    ...
    try {
      ...
      console.error("Twilio error:", error);
      return res.status(500).json({ success: false, message: "Failed to send OTP. Check Twilio details.", error: error.message });
    }
  });
  ...
  app.post("/forgot-password", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Twilio warning:", err.message);
      }
  ...
  app.post("/signup", async (req, res) => {
    const { fname, lname, password, mobile, role } = req.body;
    if (!fname || !lname || !password || !mobile) return res.status(400).json({ message: "All fields including mobile number are required" });

    if (!tempOtp[mobile] || !tempOtp[mobile].verified) return res.status(400).json({ message: "Mobile number not verified. Please verify OTP first." });
  ...
  app.post("/login", async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign({ id: user._id, role: user.role, email: user.email, mobile: user.mobile }, process.env.JWT_SECRET, { expiresIn: "24h" });
    ...
    try {
      ...
      const orderDetails = userOrders.map(order => ({
        ...
        items: order.items.map(item => ({
          productName: item.productId ? item.productId.ProductName : "Product Not Found",
          quantity: item.quantity,
          pricePerItem: item.productId ? item.productId.ProductPrice : item.priceAtBuy || 0
        }))
      }));

      return res.status(200).json({ message: "Welcome to User Dashboard", availableProducts, orders: orderDetails });
    } catch (error) {
      console.error("User dashboard error:", error);
      return res.status(500).json({ message: "Server error" });
    }
  ...
  app.get("/products/zodiac/:zodiac", async (req, res) => {
    try {
      ...
      if (category && category !== "All") query.Category = category;
      const products = await Product.find(query);
      if (!products.length) return res.status(404).json({ message: "No products found for this zodiac." });
  ...
  // Place order (secure: price validated from DB)
app.post("/place-order", authenticate, async (req, res) => {
  try {
    const { items, paymentMethod, address } = req.body;
  ...
  address
    ...
    if (paymentMethod === "card" || paymentMethod === "upi") {
      ...
      const sessionResp = await paymentHandler.orderSession({
        ...
        customer_id: req.user.id.toString(),
        customer_mobile: req.user.mobile,
        return_url: process.env.PAYMENT_CALLBACK_URL || "http://localhost:3001/payment"
      });
...
app.post("/payment-callback", async (req, res) => {
  ...
  // Get order status by customOrderId
  app.get("/order/status/:orderId", authenticate, async (req, res) => {
    try {
  ...
  app.get("/order/details/:orderId", authenticate, async (req, res) => {
    try {
      ...
      return res.json({
        ...
        paymentStatus: order.status,
        totalAmount: order.totalAmount,
        deliveryStatus: tracking?.deliveryStatus || order.orderStatus || "ORDER_PLACED",
        timeline: tracking?.timeline || []
      });
    } catch (error) {
