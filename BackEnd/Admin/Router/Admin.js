...
// Best selling products (top N) — FIXED
router.get("/stats/best-products", adminAuth, async (req, res) => {
  try {
    const topN = Math.min(50, parseInt(req.query.top || "10"));

    const bestProducts = await Order.aggregate([
      { $match: { status: "CHARGED" } }, // only paid orders
      { $unwind: "$items" },
      {
        $group: {
          _id: "$items.productId",
          productName: { $first: "$items.productName" }, // ✅ FIX
          totalQty: { $sum: "$items.quantity" },
          totalSales: {
            $sum: {
              $multiply: ["$items.quantity", "$items.priceAtBuy"]
            }
          }
        }
      },
...
router.get("/orders", adminAuth, async (req, res) => {
  try {
    ...
    const orders = await Order.find(filter)
      .sort({ createdAt: -1 })
      .limit(200)
      .populate("items.productId", "ProductName ProductPrice"); // <-- populate products
